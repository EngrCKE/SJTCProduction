/***********************
 * SJTC Production Department Dashboard v1.2.14
...
let state = {
  ...
  pendingMove: null, pendingScheduleId: null, logisticsOffsetWeeks: 0, editingLogId: null
};
...
const demo = (() => {
  ...
  return {
    settings: { ADMIN_PIN: DEMO_PIN, OFFICER_PIN: DEMO_OFFICER_PIN, APP_NAME: "SJTC Production Department Dashboard", VERSION: "1.2.14", PROCESS_COLUMNS: DEFAULT_PROCESS_COLUMNS.join("|") },
    personnel,
...
function archiveDays(){
  const raw = Number(state.settings?.ARCHIVE_DAYS_AFTER_COMPLETION || 30);
  return Number.isFinite(raw) && raw >= 0 ? raw : 30;
}
function archivedCutoffDate(){
  const d = new Date();
  d.setHours(0,0,0,0);
  d.setDate(d.getDate() - archiveDays());
  return d;
}
function statusLooksFinished(value){
  const s = String(value || "").toLowerCase();
  return /completed|complete|delivered|installed|closed|archived/.test(s);
}
function recordCompletionDate(record){
  return dateOnlyFromAny(record?.CompletedAt) ||
         dateOnlyFromAny(record?.DeliveredAt) ||
         dateOnlyFromAny(record?.CompletionDate) ||
         dateOnlyFromAny(record?.DeliveryDate) ||
         dateOnlyFromAny(record?.UpdatedAt) ||
         dateOnlyFromAny(record?.CreatedAt);
}
function isProjectArchivedByAge(project){
  if(!project || String(project.Active || "Y").toUpperCase() === "N") return true;
  if(project.ArchivedAt) return true;
  if(!statusLooksFinished(project.OverallStatus)) return false;
  const done = recordCompletionDate(project);
  return !!done && done < archivedCutoffDate();
}
function isItemArchivedByAge(item){
  if(!item || String(item.Active || "Y").toUpperCase() === "N") return true;
  if(item.ArchivedAt) return true;
  const finished = statusLooksFinished(item.DeliveryStatus) || statusLooksFinished(item.ItemStatus);
  if(!finished) return false;
  const done = recordCompletionDate(item);
  return !!done && done < archivedCutoffDate();
}
function activeProjects(){ return state.projects.filter(p=>!isProjectArchivedByAge(p)); }
function activeItems(){
  const visibleProjects = new Set(activeProjects().map(p=>String(p.ProjectID)));
  return state.items.filter(i=>visibleProjects.has(String(i.ProjectID)) && !isItemArchivedByAge(i));
}
function activeProjectIds(){ return new Set(activeProjects().map(p=>String(p.ProjectID))); }
function activeItemIds(){ return new Set(activeItems().map(i=>String(i.ItemID))); }
function currentDataOnly(){
  const pids = activeProjectIds();
  const iids = activeItemIds();
  state.projects = state.projects.filter(p=>pids.has(String(p.ProjectID)));
  state.items = state.items.filter(i=>iids.has(String(i.ItemID)));
  state.notes = state.notes.filter(n=>String(n.Active||"Y").toUpperCase()!=="N" && pids.has(String(n.ProjectID)));
  state.logs = state.logs.filter(l=>iids.has(String(l.ItemID)));
}
function projectItems(projectId){ return activeItems().filter(i=>i.ProjectID===projectId); }
...
function renderOverview(){
  const forDelivery = activeItems().filter(i=>["Delivery","Quality Control"].includes(i.ItemStatus) || i.DeliveryStatus==="Logistics Requested");
...
function renderProjects(){
  ...
  const list = activeProjects().filter(p=>[p.SONumber,p.ClientName,p.ContactNumber,p.Address,p.ProjectSummary,p.OverallStatus,teamName(p.AssignedTeamID),teamLead(p.AssignedTeamID)].join(" ").toLowerCase().includes(q)).sort((a,b)=>new Date(b.CreatedAt||0)-new Date(a.CreatedAt||0));
  $("page-projects").innerHTML = `
    <div class="pageTitle"><div><h1>Projects</h1><div class="hint">SO-level active list. Completed/delivered projects stay visible for 30 days, then move out of default views.</div></div></div>
    <div class="toolbar">
...
function renderBoard(){
  $("page-board").innerHTML = `
    <div class="pageTitle"><div><h1>Production Board</h1><div class="hint">Item-level Kanban. ${canOfficer() ? "Officer/Admin can drag/drop production movements." : "Staff view is read-only."}</div></div></div>
    ${canOfficer() ? "" : `<div class="panel readonlyNote"><span class="badge warn">Read-only Staff View</span> Drag-and-drop is disabled. Click a tile to see details.</div>`}
...
function renderKanbanCol(col){ const items = activeItems().filter(i=>i.ItemStatus===col); return `<div class="kanbanCol"><div class="kanbanHead"><span>${escapeHtml(col)}</span><span class="badge">${items.length}</span></div><div class="kanbanBody" data-status="${escapeAttr(col)}">${items.map(tileHTML).join("") || `<div class="hint">No items.</div>`}</div></div>`; }
function tileHTML(i){ const p=state.projects.find(x=>x.ProjectID===i.ProjectID)||{}; return `<div class="tile smallTile" draggable="${canOfficer()}" data-item="${escapeAttr(i.ItemID)}" data-open-item="${escapeAttr(i.ItemID)}"><div class="tileSO">${escapeHtml(i.SONumber)}</div><div class="tileItem">${escapeHtml(i.ItemDescription)}</div><div class="tileClient">${escapeHtml(p.ClientName||"")}</div><div class="tileTeam">${escapeHtml(teamName(i.AssignedTeamID))}</div></div>`; }
...
async function saveProductionLogEdit(){
  ...
  await api("editProductionLog", { pin: state.pin, logId, updates, CorrectionNote: correctionNote, editedBy:"Admin" });
  closeModal("editLogModal");
  state.editingLogId = null;
  await load({silent:true});
  if(log.ItemID) openItemDetails(log.ItemID);
}
...
async function confirmMove(){
  ...
  await api("moveProductionItem", { pin:accessPin(), itemId:state.pendingMove.itemId, toStatus:state.pendingMove.toStatus, assignedPersonnel, remarks:$("moveRemarks").value.trim(), movedBy:"Admin" });
...
function selectableDeliveryItems(projectId){
  return projectItems(projectId).filter(i=>i.Active!=="N" && !["Delivered","Installed","Cancelled"].includes(String(i.DeliveryStatus||"")));
}
function itemDefaultChecked(i){
  return ["Delivery","Quality Control"].includes(String(i.ItemStatus||"")) || ["Not Requested","Logistics Requested"].includes(String(i.DeliveryStatus||"")) && String(i.ItemStatus||"")==="Delivery";
}
...
function autoFillDeliveryFromSO(preselectedIds=[]){
  ...
  if($("lrAddress")) $("lrAddress").value = project.Address || "";
...
function editVehiclePassenger(id=""){
  ...
  openSettingsForm(vp.PassengerID?"Passenger / Crew Details":"Add Default Passenger / Crew",[
    ...
    {key:"Active",label:"Active",type:"select",options:["Y","N"]}
  ], Object.assign({Active:"Y"},vp), async(data)=>api("upsertVehiclePassenger",{pin:accessPin(), vehiclePassenger:Object.assign({},vp,data)}));
}
...
function renderSettings(){
  ...
  const visibleSettings = Object.entries(state.settings||{}).filter(([k])=>k !== "ADMIN_PIN" && k !== "OFFICER_PIN");
  ...
  const settingsBody = `<div class="sectionActions"><button class="primary" onclick="editSetting('ADMIN_PIN','Admin PIN','The current PIN is never displayed. Enter a new PIN only if you want to change it.')">Change Admin PIN</button><button class="primary" onclick="editSetting('OFFICER_PIN','Officer PIN','Officer PIN allows production board movement and logistics scheduling without full admin access.')">Change Officer PIN</button></div>` + settingsList(safeSettings, "No app settings yet.");
...
function renderAbout(){ $("page-about").innerHTML = `<div class="pageTitle"><div><h1>About</h1><div class="hint">System information and credits.</div></div></div><div class="panel aboutBox"><h2>SJTC Production Department Dashboard</h2><p>A production and logistics coordination system for monitoring projects, tracking item-level production progress, managing partial delivery batches, and coordinating logistics requests.</p><p><b>Version:</b> 1.2.14<br><b>Company:</b> SJTC Manufacturing Inc. / Focolare Carpentry</p><p><b>Developed by:</b> Engr. CK Empeynado</p><p class="small">This system uses GitHub Pages for the frontend, Cloudflare Worker as proxy, Google Apps Script as API, and Google Sheets as database. It is intended for internal production monitoring and logistics coordination.</p></div>`; }
