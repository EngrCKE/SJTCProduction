...
const demo = (() => {
  ...
  const personnel = [
    { PersonnelID:"PER-0001", PersonnelName:"Juan D.", Role:"Team Lead", Active:"Y" },
    { PersonnelID:"PER-0002", PersonnelName:"Mark", Role:"Carpenter", Active:"Y" },
    { PersonnelID:"PER-0003", PersonnelName:"Allan", Role:"Helper", Active:"Y" },
    { PersonnelID:"PER-0004", PersonnelName:"Rico", Role:"Painter", Active:"Y" },
...
function logisticsCard(r){
  const p=r.Payload||{};
  return `<div class="card logisticsCard" draggable="${state.admin}" data-logreq="${escapeAttr(r.RequestID)}" data-open-logreq="${escapeAttr(r.RequestID)}">
    <div class="cardTitle">${escapeHtml(cleanSO(p.SONumber)||r.RequestID)} • ${escapeHtml(p.ClientName||displayType(r.Type)||"")}</div>
    <div class="small">${escapeHtml(displayType(r.Type))} • ${escapeHtml(p.Address||p.Destination||"")}</div>
    <div class="meta"><span class="pill ${r.Status==="PENDING"?"warn":"ok"}">${escapeHtml(r.Status||"—")}</span><span>${niceDT(r.StartDT)}</span><span>${escapeHtml(r.DriverCode||"No driver")}</span><span>${escapeHtml(r.VehicleCode||"No vehicle")}</span></div>
  </div>`;
}

function renderProjects(){
  const old = $("projectSearch"); const q = (old && old.value || "").toLowerCase();
  const list = state.projects.filter(p=>p.Active!=="N" && [p.SONumber,p.ClientName,p.ContactNumber,p.Address,p.ProjectSummary,p.OverallStatus,p.AssignedTeamLead].join(" ").toLowerCase().includes(q)).sort((a,b)=>new Date(b.CreatedAt||0)-new Date(a.CreatedAt||0));
  $("page-projects").innerHTML = `
    ...
    <div class="toolbar">
      ...
      <input id="projectSearch" placeholder="Search SO#, client, address, item, team lead..." value="${escapeAttr(q)}" />
    </div>
    <div class="panel"><table><thead><tr><th>SO#</th><th>Client</th><th>Contact</th><th>Address</th><th>Item Description</th><th>Due Date</th><th>Status</th><th>Team Lead</th><th>Days to Due</th></tr></thead><tbody>
      ${list.map(p=>{ const due=daysToDue(p.DueDate); const items=projectItems(p.ProjectID); return `<tr class="click" data-project="${escapeAttr(p.ProjectID)}"><td><b>${escapeHtml(p.SONumber)}</b></td><td>${escapeHtml(p.ClientName)}</td><td>${escapeHtml(p.ContactNumber||"")}</td><td>${escapeHtml(p.Address||"")}</td><td>${escapeHtml(p.ProjectSummary||items.map(i=>i.ItemDescription).join(", "))}<div class="small">${items.length} item${items.length!==1?"s":""}</div></td><td>${niceDate(p.DueDate)}</td><td>${escapeHtml(p.OverallStatus||"")}</td><td>${escapeHtml(p.AssignedTeamLead||"—")}</td><td><span class="pill ${due.cls}">${due.text}</span></td></tr>`; }).join("")}
    </tbody></table></div>`;
...
function openProjectDetails(projectId){
  ...
  $("projectModalBody").innerHTML = `
    <div class="grid2"><div class="panel"><h3>Project Information</h3>
      ${detail("SO#",p.SONumber)}${detail("Client",p.ClientName)}${detail("Contact Number",p.ContactNumber)}${detail("Address",p.Address)}${detail("Due Date",niceDate(p.DueDate))}${detail("Overall Status",p.OverallStatus)}${detail("Assigned Team Lead",p.AssignedTeamLead)}${detail("Team",teamName(p.AssignedTeamID))}
...
function openProjectForm(projectId){
  ...
  $("projectFormBody").innerHTML = `
    <div class="twoCol">
      ...
      <div><label>Assigned Team</label><select id="fAssignedTeamID"><option value="">None</option>${state.teams.filter(t=>t.Active!=="N").map(t=>`<option value="${escapeAttr(t.TeamID)}" ${p.AssignedTeamID===t.TeamID?"selected":""}>${escapeHtml(t.TeamName)} — ${escapeHtml(t.TeamLead)}</option>`).join("")}</select></div>
      ${inputField("AssignedTeamLead","Assigned Team Lead",p.AssignedTeamLead)}
    </div>
...
function pendingLogisticsCard(r){ const p=r.Payload||{}; return `<div class="card logisticsCard" draggable="${state.admin}" data-logreq="${escapeAttr(r.RequestID)}" data-open-logreq="${escapeAttr(r.RequestID)}"><div class="cardTitle">${escapeHtml(cleanSO(p.SONumber)||r.RequestID)} • ${escapeHtml(p.ClientName||"")}</div><div class="small">${escapeHtml(displayType(r.Type))} • ${escapeHtml(p.Address||p.Destination||"")}</div><div class="meta"><span class="pill warn">${escapeHtml(r.Status)}</span><span>${niceDT(r.StartDT)}</span></div></div>`; }
...
function driverOptions(selected=""){
  return `<option value="">Select driver</option>` + state.drivers.map(d=>{
    const label=d.DriverName || personnelName(d.PersonnelID) || d.DriverID;
    const val=d.DriverName || d.DriverID;
    return `<option value="${escapeAttr(val)}" ${String(selected)===String(val)?"selected":""}>${escapeHtml(label)}</option>`;
  }).join("");
}
function vehicleOptions(selected=""){
  return `<option value="">Select vehicle</option>` + state.vehicles.map(v=>{
    const val=v.VehicleCode || v.VehicleID;
    return `<option value="${escapeAttr(val)}" ${String(selected)===String(val)?"selected":""}>${escapeHtml(val)} — ${escapeHtml(v.VehicleLabel||"")}</option>`;
  }).join("");
}
function personnelName(id){ const p=state.personnel.find(x=>x.PersonnelID===id); return p ? p.PersonnelName : ""; }
...
function passengerPicker(selectedNames=[]){
  ...
  return `<div class="personnelPicker">${state.personnel.filter(p=>String(p.Active||"Y").toUpperCase()!=="N").map(p=>{
    const checked=selectedSet.has(p.PersonnelName) || selectedSet.has(p.PersonnelID);
    return `<label class="personnelPick"><input type="checkbox" class="schedPassenger" value="${escapeAttr(p.PersonnelID)}" data-name="${escapeAttr(p.PersonnelName)}" ${checked?"checked":""}/> <span>${escapeHtml(p.PersonnelName)} <span class="small">${escapeHtml(p.Role||"")}</span></span></label>`;
  }).join("")}</div>`;
}
function normalizePassengers(payload){
  const raw = payload && (payload.Passengers || payload.Installers || payload.Personnel || "");
  if(Array.isArray(raw)) return raw.map(x=> typeof x === "object" ? (x.PersonnelName || x.InstallerName || x.name || x.Name || "") : String(x)).filter(Boolean);
...
function openScheduleModal(requestId,date){
  ...
  const passengers = normalizePassengers(r.Payload||{});
...
function openLogisticsDetailModal(requestId){
  ...
  $("logisticsDetailTitle").textContent = `Logistics Request ${r.RequestID || ""}`;
  const passengers = normalizePassengers(p).join(", ") || "—";
  const scheduleHtml = `<div class="requestDetailBox"><h3>Schedule</h3>${detail("Date", r.StartDT ? new Date(r.StartDT).toLocaleDateString() : "—")}${detail("Time", r.StartDT && r.EndDT ? `${new Date(r.StartDT).toLocaleTimeString(undefined,{hour:"2-digit",minute:"2-digit"})}–${new Date(r.EndDT).toLocaleTimeString(undefined,{hour:"2-digit",minute:"2-digit"})}` : "—")}${detail("Driver", r.DriverCode || "—")}${detail("Vehicle", r.VehicleCode || "—")}${detail("Passengers", passengers)}${detail("Trip", r.TripStatus || "READY")}</div>`;
  const requestHtml = `<div class="requestDetailBox"><h3>Request</h3>${detail("Type", displayType(r.Type))}${detail("Status", r.Status)}${detail("Requested By", r.RequestedBy)}${detail("SO#", cleanSO(p.SONumber)||"—")}${detail("Client", p.ClientName||"—")}${detail("Contact", p.ContactNumber||"—")}${detail("Address/Destination", p.Address||p.Destination||"—")}${detail("Area", p.AreaClass||"—")}${detail("Items", p.Items||"—")}${detail("Purpose", p.Purpose||"—")}${detail("Required Vehicle", p.RequiredVehicle||"—")}${detail("Notes", p.Notes||"—")}</div>`;
...
function renderSettings(){
  $("page-settings").innerHTML = `<div class="pageTitle"><div><h1>Settings</h1><div class="hint">Admin-maintained parameters and lists.</div></div></div><div class="grid2">
    <div class="panel"><h3>Production Process Columns</h3>${PROCESS_COLUMNS.map(x=>`<span class="pill info" style="margin:3px">${escapeHtml(x)}</span>`).join("")}</div>
    <div class="panel"><h3>Team Management</h3>${state.teams.map(t=>`<div class="card"><div class="cardTitle">${escapeHtml(t.TeamName)}</div><div class="small">Lead: ${escapeHtml(t.TeamLead)}</div><div class="small">Members: ${state.teamMembers.filter(m=>m.TeamID===t.TeamID&&m.Active!=="N").map(m=>m.MemberName).join(", ")||"—"}</div></div>`).join("")||`<div class="hint">No teams yet.</div>`}</div>
    <div class="panel"><h3>Personnel List</h3>${state.personnel.map(p=>`<div class="card"><div class="cardTitle">${escapeHtml(p.PersonnelName)}</div><div class="small">${escapeHtml(p.Role||"Personnel")}</div></div>`).join("") || `<div class="hint">No personnel records yet.</div>`}</div>
    <div class="panel"><h3>Drivers</h3>${state.drivers.map(d=>`<div class="card"><div class="cardTitle">${escapeHtml(d.DriverName||d.DriverID)}</div><div class="small">${escapeHtml(d.DriverPhone||"")}</div></div>`).join("") || `<div class="hint">No drivers yet.</div>`}</div>
    <div class="panel"><h3>Vehicles</h3>${state.vehicles.map(v=>`<div class="card"><div class="cardTitle">${escapeHtml(v.VehicleCode||v.VehicleID)}</div><div class="small">${escapeHtml(v.VehicleLabel||"")} • Plate: ${escapeHtml(v.PlateNo||"—")}</div></div>`).join("") || `<div class="hint">No vehicles yet.</div>`}</div>
    <div class="panel"><h3>Passenger Defaults</h3>${state.vehiclePassengers.map(vp=>`<div class="card"><div class="cardTitle">${escapeHtml(vp.PassengerName||vp.PersonnelID)}</div><div class="small">Vehicle: ${escapeHtml(vp.VehicleID||"—")}</div></div>`).join("") || `<div class="hint">No passenger defaults yet.</div>`}</div>
    <div class="panel"><h3>Parameter Groups</h3><div class="small">Admin PIN • Personnel List • Project Defaults • Logistics Defaults • Announcements • Database/App Settings</div></div>
