/***********************
 * SJTC Production Department Dashboard v1.2.6
...
const demo = (() => {
  ...
  return {
    settings: { ADMIN_PIN: DEMO_PIN, APP_NAME: "SJTC Production Department Dashboard", VERSION: "1.2.6", PROCESS_COLUMNS: DEFAULT_PROCESS_COLUMNS.join("|") },
    personnel,
...
function passengerPicker(selectedNames=[]){
  ...
  return `<div class="personnelDeptPicker">${groups.map((g,idx)=>`
    <details class="deptPickGroup" ${idx===0?"open":""}>
      <summary>${escapeHtml(g.dept)} <span class="badge">${g.items.length}</span></summary>
      <div class="personnelPicker compactDeptPicker">
        ${g.items.map(p=>{ const checked=selectedSet.has(p.PersonnelName) || selectedSet.has(p.PersonnelID); return `<label class="personnelPick"><input type="checkbox" class="schedPassenger" value="${escapeAttr(p.PersonnelID)}" data-name="${escapeAttr(p.PersonnelName)}" ${checked?"checked":""}/> <span>${escapeHtml(p.PersonnelName)} <span class="small">${escapeHtml(p.Role||"")}</span></span></label>`; }).join("")}
      </div>
    </details>`).join("")}</div>`;
}
...
function editTeam(id=""){
        ...
        <div class="memberPickerList groupedMemberPicker">
          ${groupByDepartment(activePersonnelSorted()).map(g=>`<details class="deptPickGroup" open><summary>${escapeHtml(g.dept)} <span class="badge">${g.items.length}</span></summary>${g.items.map(p=>`
            <label class="memberPickRow">
              <input type="checkbox" class="teamMemberCheck" value="${escapeAttr(p.PersonnelID)}" ${isMemberChecked(p)?"checked":""} ${state.admin?"":"disabled"} />
            ...
            </label>`).join("")}</details>`).join("") || `<div class="hint">No personnel records yet.</div>`}
        </div>
      </div>
    </div>`;
  ...
  if(btn) btn.onclick = e => runAction(e,"Saving team and members...", async()=>{
    ...
    const checked = Array.from(document.querySelectorAll(".teamMemberCheck:checked")).map(ch => {
      const p = state.personnel.find(x=>String(x.PersonnelID)===String(ch.value)) || {};
      return {
        PersonnelID: p.PersonnelID || ch.value,
        MemberName: personName(p),
        Role: personRole(p),
...
function renderAbout(){ $("page-about").innerHTML = `<div class="pageTitle"><div><h1>About</h1><div class="hint">System information and credits.</div></div></div><div class="panel aboutBox"><h2>SJTC Production Department Dashboard</h2><p>A production and logistics coordination system for monitoring projects, tracking item-level production progress, managing partial delivery batches, and coordinating logistics requests.</p><p><b>Version:</b> 1.2.6<br><b>Company:</b> SJTC Manufacturing Inc. / Focolare Carpentry</p><p><b>Developed by:</b> Engr. CK Empeynado</p><p class="small">This system uses GitHub Pages for the frontend, Cloudflare Worker as proxy, Google Apps Script as API, and Google Sheets as database. It is intended for internal production monitoring and logistics coordination.</p></div>`; }
